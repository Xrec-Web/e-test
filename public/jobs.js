...
let fetchJobs = async () => {
  try {
    const response = await fetch(
      'https://w-scripts.vercel.app/api/loxo/empowered-recruitment/jobs'
    );
...
let fetchJob = (jobId, element) => {
  ...
  fetch(
    'https://w-scripts.vercel.app/api/loxo/empowered-recruitment/job?id=' +
      jobId,
    options
  )
...
var createItem = (job, jobDescription, templateElement) => {
  ...
  if (urlLink)
    urlLink.href = 'https://empoweredrecruitment.com.au/job?id=' + job.id;
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  Webflow.push(function () {
    $('#wf-form-Job-Apply-Form').submit(async function (e) {
      ...
      fetch(
        'https://w-scripts.vercel.app/api/loxo/empowered-recruitment/apply',
        options
      )
        .then((response) => {
